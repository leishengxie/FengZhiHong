...
const QString c_strMainAppName = "BlessCoin_b.exe";
...
void MainWindow::onDownloadCompleted(T_TaskDownloadInfo tTaskDownloadInfo)
{
...
void MainWindow::onFinishedAllTask()
{
    ui->labelTitle->setText("更新完成……");
    executeMainApp(c_strMainAppName);
}

bool MainWindow::checkUpdateFiles(QString strNewFile, QString strOldFile)
{
    ...
    for(int i=0; i<nodeList.size(); i++)
    {
        ...
        T_BinFileMsg tBinFileMsgOld = getElementMsgByName(strOldFile, tBinFileMsgNeW.name);//获取本地xml文件对应文件（name）的版本信息
...
T_BinFileMsg MainWindow::getElementMsgByName(QString strXmlFile, QString strBinName)
{
    T_BinFileMsg tBinFileMsg;
...
// 确认哪些文件加入下载列表
void MainWindow::checkVersion(const T_BinFileMsg &tBinFileMsgNew, const T_BinFileMsg &tBinFileMsgOld)
{
    // 本地XML没有此文件,加入下载列表
...
// 根据更新列表下载
void MainWindow::downLoadUpdateFiles()
{
    ui->labelTitle->setText("正在下载更新文件……");
    QStringList urlsStringList;
    foreach (T_BinFileMsg tBinFileMsg, m_lstBinFileMsg)
    {
...
void MainWindow::executeMainApp(QString strMain)
{
    if(!strMain.isEmpty())
    {
...
void MainWindow::onDownloadProgressAll(qint64 bytesReceived, qint64 bytesTotal)
{
