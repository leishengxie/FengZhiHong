    ...
    , m_eRequestStatus(ER_WaitForRequest)
{
    ...
    m_nMaxSizeToRead = 16000;
    m_nMaxMultiPartSize = 1000000;
    tempFile = NULL;
}
...
void CLHttpRequest::read()
{
    ...
    if ((m_bytesBoundary.isEmpty() && m_nCurrentSize > m_nMaxSizeToRead) || (!m_bytesBoundary.isEmpty() && m_nCurrentSize > m_nMaxMultiPartSize))
    {
...
void CLHttpRequest::readRequestLine()
{
    ...
    int toRead = m_nMaxSizeToRead - m_nCurrentSize + 1; // allow one byte more to be able to detect overflow 检查溢出
    bytesLine.append(m_pHttpConnection->readLine(toRead)); // 读取字节
    m_nCurrentSize += bytesLine.size();

    if (!bytesLine.contains('\r') && !bytesLine.contains('\n'))
    {
        return;
    }

    QByteArray newData = bytesLine.trimmed(); // 去除两端空格
...
{
    int toRead = m_nMaxSizeToRead - m_nCurrentSize + 1;
    QByteArray lineBuffer;
    lineBuffer.append(m_pHttpConnection->readLine(toRead));
    m_nCurrentSize += lineBuffer.size();
    ...
    else
    {
        ...
        QByteArray bytesContentType = m_mapHeaders.value("content-type");

        if (bytesContentType.startsWith("multipart/form-data"))
        {
            int posi = bytesContentType.indexOf("boundary=");

            if (posi >= 0 )
            {
                m_bytesBoundary = bytesContentType.mid(posi + 9);

                if  (m_bytesBoundary.startsWith('"') && m_bytesBoundary.endsWith('"'))
                {
                    m_bytesBoundary = m_bytesBoundary.mid(1, m_bytesBoundary.length() - 2);
                }
            }
        }
        ...
        {
            m_nExpectedBodySize = contentLength.toInt();
        }

        if (m_nExpectedBodySize == 0)
        {
            qDebug("HttpRequest: expect no body");
            m_eRequestStatus = ER_Complete;
        }
        else if (m_bytesBoundary.isEmpty() && m_nExpectedBodySize + m_nCurrentSize > m_nMaxSizeToRead)
        {
            qWarning("HttpRequest: expected body is too large");
            m_eRequestStatus = ER_Abort;
        }
        else if (!m_bytesBoundary.isEmpty() && m_nExpectedBodySize > m_nMaxMultiPartSize)
        {
        ...
        {
            qDebug("HttpRequest: expect %i bytes body", m_nExpectedBodySize);
...
{
    Q_ASSERT(m_nExpectedBodySize != 0);

    if (m_bytesBoundary.isEmpty())
    {
        // normal body, no multipart
        qDebug("HttpRequest: receive body");
        int toRead = m_nExpectedBodySize - bodyData.size();
        QByteArray newData = m_pHttpConnection->read(toRead);
        m_nCurrentSize += newData.size();
        bodyData.append(newData);

        if (bodyData.size() >= m_nExpectedBodySize)
        {
            m_eRequestStatus = ER_Complete;
        }
    }
    else
    {
        ...
        int toRead = m_nExpectedBodySize - fileSize;
        ...
        if (fileSize >= m_nMaxMultiPartSize)
        {
            qWarning("HttpRequest: received too many multipart bytes");
            m_eRequestStatus = ER_Abort;
        }
        else if (fileSize >= m_nExpectedBodySize)
        {
...
void CLHttpRequest::parseMultiPartFile()
{
    ...
    while (!tempFile->atEnd() && !finished && !tempFile->error())
    {
        ...
        while (!tempFile->atEnd() && !finished && !tempFile->error())
        {
            ...
            if (line.startsWith("--" + m_bytesBoundary))
            {
                ...
                if (line.contains(m_bytesBoundary + "--"))
                {
                    finished = true;
                }
