...
///
/// \brief Worker::processQueuedConnections
///
...
//            , this, SLOT(onNewRequest(CLHttpRequest*, CLHttpResponse*)));
    ...
    connect(conn, SIGNAL(disconnected()), this, SLOT(onDone()));
}
...
void CLWorkerThread::run()
{
    ...
    if(m_sockets.size() > 0)
    {
        pWorker->m_sockets = m_sockets;
        m_sockets.clear();
        QCoreApplication::postEvent(pWorker, new TaskEvent(0));
    }
    exec();
}
