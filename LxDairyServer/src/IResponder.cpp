...
//http 常见的三种互联网媒体类型content-type
// application/x-www-form-urlencoded // 在最开始的请求方式中，请求参数都是放在url中，表单提交的时候，都是以key=&value=的方式写在url后面
// multipart/form-data // 此种方式多用于文件上传，表单数据都保存在http的正文部分，各个表单项之间用boundary分开
// text/plain： 数据以纯文本形式(text/json/xml/html)进行编码，其中不含任何控件或格式字符。

// application/json //现在越来越多的应用使用application/json,用来告诉服务端消息主体是序列化的json字符串。
...
IResponder::IResponder(CLHttpRequest* req, CLHttpResponse* resp)
    : m_req(req)
    , m_resp(resp)
{
}

bool IResponder::isForbidden(const QString & uriPath)
{
    int count = m_strlstForbiddenPatter.size();

    for(int i = 0; i < count; i++)
    {
        const QString & patter = m_strlstForbiddenPatter.at(i);

        if(uriPath.startsWith(patter))
        {
            return true;
        }
    }
