...
void ColorMaker::start()
{
    if(m_nColorTimer == 0)
    {
        m_nColorTimer = startTimer(1000);    //开启1秒中断，执行timerevent
    }
}
...
void ColorMaker::nextAlgorithm()
{
		m_algorithm = (GenerateAlgorithm)(((int)m_algorithm + 1) % 5);
		emit algorithmChanged(m_algorithm);
}

void ColorMaker::timerEvent(QTimerEvent *e)              //time事件，通过starttime开始
{
    if(e->timerId() == m_nColorTimer)          //判断是否这个？？
    {
        switch(m_algorithm)
        {
        case RandomRGB:
